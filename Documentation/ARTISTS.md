...
This guide is for designers, level artists, and tech artists authoring
items in the editor. Zero code required. If you can drop a texture into
a slot, you can ship an item.

> If you need to wire a NEW kind of item effect (a poison status, a
> deployable shield, a thrown grenade), that's a code change — see
...
1. In the Content Browser, navigate to where you want the asset to live
   (e.g. `Assets/Items/Health/`).
2. Right-click → **Create Asset → Items → ItemInfo**.
3. Name it. Prefix conventions used in the FPS Demo:
   - `I_Ammo_*`   — ammo packs
   - `I_Health_*` — medkits / stim packs
   - `I_Shield_*` — shield cells
   - `I_Weapon_*` — weapon pickups
   - `I_Key_*`    — quest / key items
   - `I_Coin_*`   — currency / pickups with score value
4. Double-click to open the inspector.

The asset's chip in the Content Browser is **gold** — that's how you
spot an `ItemInfo` versus a `WeaponInfo` (orange) or `LootTable` (cyan).
...
|---|---|---|
| **Item Name** | The string other systems (loot tables, pickup actors, Lua) use to find this item. | Leave blank to use the asset name. Set it explicitly when you need to rename the asset file without breaking references everywhere. |
| **Display Name** | The pretty name shown in HUD pickups, inventory grids, tooltips. | "Small Medkit", not "I_Health_Small". |
| **Description** | One-paragraph flavor / mechanical text for tooltips and inspect screens. | Designers can rewrite this without touching code. |
| **Category** | Integer that picks the effect handler. **See cheat-sheet below.** | Change this and the inspector "CategoryData" fields take on new meaning — re-read the per-category sections. |
...
| 3 | Weapon | Grants / tops up a weapon archetype |
| 4 | KeyItem | No built-in effect — pure signal + inventory entry |
| 5 | Currency | No built-in effect — pure signal + inventory entry |
| 6 | Generic | No built-in effect — pure signal + inventory entry |

If you set Category to a number not in this list, the asset loads fine
but `Items.Apply` will warn and refuse to apply it.
...
| **Equipable** | Marks the item as something the player can equip (vs. just consume). Used by the inventory UI to show an "Equip" verb. Weapons usually leave this `false` because the Weapon category handles equipping itself. |
| **Consumable** | Marks the item as consumable for inventory UI / loot filters. |
| **Throwable** | Marks the item as throwable. The pickup script's "toss" path fires `onTossSignal`. |

### CategoryData

These fields **change meaning** depending on the Category you chose
above. The inspector shows all three fields all the time — just ignore
the ones that don't apply to your category.
...
| | KeyItem / Generic | Free-form metadata; no engine reads it |
...
- **Exact name** (e.g. `WI_Pistol`) — only refills that specific weapon
  ...
  and the handler returns false — i.e. the pickup script will leave it
  on the ground.

### Signals

Empty = fall back to the project defaults:
...
hook — e.g. `FPS.Quest.AncientKey.Found` for a story item that should
trigger a cinematic. Anything listening on the SignalBus to that name
will fire.
...
All five are optional — leave them blank and that step is skipped.
...
gains 30 rounds. If not, the pickup stays on the ground (returns false).
...
- If the player isn't carrying a shotgun and has a free slot: equip the
  shotgun, give it 12 reserve, auto-swap to it, fire `FPS.Item.Equipped`.
- If the player is already carrying a shotgun: top up reserve by 12.
- If all weapon slots are full and they don't have a shotgun: pickup
  stays on the ground.
...
Doesn't auto-use, doesn't stack. Picking it up just adds it to inventory
and emits the custom signal — quest scripts listening to that signal
...
Touch to collect. Score / wallet systems listening on `FPS.Item.Pickup`
read `amount` from the asset to add value.

---

## 4. Asset hygiene

- **Don't rename `Item Name` for an item that's already in saves /
...
- **Category changes are destructive** — flipping `Health` to `Weapon`
  silently reinterprets the `Amount`/`Ammo For Weapon`/`Weapon Archetype`
  fields. Re-validate the asset after any category change.
- **Live edits propagate.** While the editor is running, edits to an
  `ItemInfo` re-register it in the registry on save — Lua callers
  picking the asset up after the save see the new values without a
...
|---|---|
| `Items.Apply: no ItemInfo registered for 'X'` in log | The `Item Name` field doesn't match what the caller asked for — check spelling, or asset wasn't loaded. Try calling `Items.LoadAllItems()` from the Lua console. |
...
| Heal / ammo gives the wrong amount | `Amount` is per-stack and gets multiplied by the pickup's `count`. A medkit `Amount=25` picked up as a stack-of-2 heals 50. |
...
| Custom signal doesn't fire | The asset's signal field is blank → falls back to default. Fill it in. Also check that whatever's listening on the SignalBus subscribed to the right name. |
...
- Loot tables (chance-based grants from chests / enemies):
  `com.polyphase.system.rpg.loot` package.
- Weapon archetypes (what `Weapon Archetype` points at):
  `com.polyphase.engine.combat.core` package.
- Scripting your own item effect: [DEVELOPERS.md](DEVELOPERS.md).
